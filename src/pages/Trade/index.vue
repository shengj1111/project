  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      >
        ...
        <p @click="changeDefault(userAddress, address)">
...
export default {
  ...
  mounted() {
    ...
    this.$store.dispatch('getUserTradeCart')
  },
  computed: {
    ...mapState({
      address: (state) => state.trade.userAddress,
      tradeCartInfo: (state) => state.trade.tradeCartInfo,
    }),
    //用户下单地址
    userDefaultAddress() {
    ...
    //商品展示
    detailArrayList() {
      return this.tradeCartInfo.detailArrayList || []
    },
    //总共价钱
    totalPrice() {
      let sum = 0
  ...
  methods: {
    //改变地址
    changeDefault(userAddress, address) {
    ...
    async submitOrder() {
      let { tradeNo } = this.tradeCartInfo //订单编号
      let data = {
        consignee: this.userDefaultAddress.consignee, //用户名字
        consigneeTel: this.userDefaultAddress.phoneNum, //用户电话
        deliveryAddress: this.userDefaultAddress.fullAddress, //用户地址
        paymentWay: 'ONLINE', //支付方式
        orderComment: this.msg, //买家留言
        orderDetailList: this.detailArrayList, //商品信息
      }
      //发送请求
      let result = await this.$API.reqSumbitOrder(tradeNo, data)
      if (result.code == 200) {
